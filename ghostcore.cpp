// ghostcore.cpp - Main File for GhostShell
// File Path: ghostshell/ghostcore.cpp

#include "ghostcore.h"
#include <iostream>
#include <oqs/oqs.h>

// Constructor for GhostCore
GhostCore::GhostCore()
    ...
    oqsSig(OQS_SIG_new(OQS_SIG_alg_dilithium_2), OQS_SIG_free)
{
    ...
    if (oqsSig == nullptr) {
        errorHandler.HandleError("GhostCore Constructor", "Failed to initialize Dilithium signature mechanism.");
    }
    OQS_init();
}

// Destructor for GhostCore
GhostCore::~GhostCore() {
...
// Main function - Entry point for GhostShell
int main() {
    GhostCore core;
    core.Run();
    return 0;
